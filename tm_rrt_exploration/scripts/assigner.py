...
def node():
    ...
    if len(robot_namelist) > 0:
        for i in range(0, len(robot_namelist)):
            ...
            robots.append(robot(name=robot_namelist[i], move_base_service=move_base_service, in_service=plan_service,
                                global_frame=global_frame, base_link=base_link))
    ...
    while not rospy.is_shutdown():
        if resetSignal:
            ...
            if len(robot_namelist) > 0:
                for i in range(0, len(robot_namelist)):
                    robots.append(
                        robot(name=robot_namelist[i], move_base_service=move_base_service, in_service=plan_service,
                              global_frame=global_frame, base_link=base_link))
        ...
        if startStopSignal:
            ...
            for ii in range(0, len(robot_namelist)):
                ...
                if movebase_status >= 4 and robot_assigned_goal[ii]['time_thres'] != -1:
                    ...
                    rospy.loginfo(
                        ...
                        + " at time: " + str(currTime) + " sec  -- distance: " + str(
                            distance) + " -- robot path planning failed \n")
            ...
            for qq in range(0, len(robot_namelist)):
                ...
                if debugFlag2:
                    print("Robot %s goal and position queried used time: %fs" % (
                    robot_namelist[qq], time.time() - startTime))
            ...
            for ir in robotListTemp2:
                ...
                if debugFlag2:
                    print("Robot %s revenue and centroid calculated used %fs" % (
                    robot_namelist[ir], time.time() - startTime))
            # -------------------------------------------------------------------------
                ...
                if len(na) >= 1:
                    if debugFlag1:
                        ...
                        for ih in range(0, len(robot_namelist)):
                            currTime = rospy.get_rostime().secs
                            timeRemaining = (robot_assigned_goal[ih]['time_start'] + robot_assigned_goal[ih][
                                'time_thres']) - currTime
                            rospy.loginfo("robot " + robot_namelist[ih] + " - last goal:" + str(
                                robot_assigned_goal[ih]['lastgoal']))
                else:
                    ...
                    for ih in range(0, len(robot_namelist)):
                        ...
                        timeRemaining = (robot_assigned_goal[ih]['time_start'] + robot_assigned_goal[ih][
                            'time_thres']) - currTime
                        rospy.loginfo(
                            "robot " + robot_namelist[ih] + " working on goal:" + str(robot_assigned_goal[ih]['goal'])
            ...
            if (len(centroids) > 0):
                ...
                if rospy.get_rostime().secs >= next_assign_time:
                    for xxx in robotListTemp:
                        ...
                        skipAssign = False
                        if xxx in nb:
                            goalDistance = calculateLocationDistance(robots[xxx].getPosition(),
                            ...
                            if goalDistance >= (hysteresis_radius) and goalDistance <= (rp_metric_distance * 1.5):
                                ...
                                if debugFlag1:
                                    print(
                                                "\n ======== --- Robot %s skip the goal assignment!  Distance to goal: %fm" % (
                                        robot_namelist[xxx], goalDistance))
                            # now check the time.
                            if timeElapsed > non_interrupt_time and goalDistance <= hysteresis_radius:
                                ...
                                if debugFlag1:
                                    print(
                                                "\n ======== --- Robot %s skip the goal assignment!  non interrupt time: %.2fs" % (
                                        robot_namelist, (non_interrupt_time - timeElapsed)))
                        if not skipAssign:
                            ...
                            if (cond_history or cond_goalTaken) and not cond_goal:
                                ...
                                if goal_repeated:
                                    ...
                                    rospy.loginfo("\n !!!> Robot " + robot_namelist[xxx] + " give up goal: " + str(
                                        robot_assigned_goal[xxx]['goal'])
                                                  + " at time: " + str(int(currTime)) + " sec  -- distance: " + str(
