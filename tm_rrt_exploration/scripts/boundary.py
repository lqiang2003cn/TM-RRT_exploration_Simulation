...
def boundaryMapInfoCallback(data):
    ...
    if len(boundary_Pts) < 4:
        boundary_Pts.append(np.array([((map_width * map_resolution) * -1.0) - pos_origin_x + 0.01,
...
def node():
    ...
    topicInput = rospy.get_param('~topicInput',
                                 '/clicked_point')  # the number of the bXXoundary point going to be created
    ...
    controlOutput = rospy.get_param('~controlOutput',
                                    '/explore_start')  # the number of the boundary point going to be created
    restartOutput = rospy.get_param('~restartOutput',
                                    '/explore_reset')  # the number of the boundary point going to be created
    topicOutput = rospy.get_param('~topicOutput',
                                  '/exploration_boundary')  # specify the topic which the node is going to published into the list
    frequency = rospy.get_param('~frequency', 2.0)  # number of the rate listening to the ros param
    timeInterval = rospy.get_param('~timeInterval',
                                   5.0)  # time interval for broadcasting the information on odometry and time elapsed
    autoInputPoint = rospy.get_param('~AutoInputPoint',
                                     "")  # the auto input point instead of clicked point. (points separated by semicolon; and axis are separated by comma,)
    diagDistance = rospy.get_param('~diagonal_distance',
                                   7.50)  # the auto input point instead of clicked point. (points separated by semicolon; and axis are separated by comma,)
                ...
                if valid_entry:
                    ...
                    while startExploration:
                        ...
                        if nextTimeReport <= rospy.get_rostime().secs:
                            rospy.loginfo('--- >>> current time: %.2f    time elapsed: %.2f' % (
